...
FRONTEND_INTERFACES: dict[type, set[str]] = {
    PortfolioResponse: {
        "id", "user_id", "name", "investment_purpose", "investment_horizon",
        "cash_ratio", "created_at", "updated_at",
    },
    HoldingResponse: {
        "id", "portfolio_id", "ticker", "company_name", "quantity",
        "avg_buy_price", "target_weight", "current_weight", "created_at", "updated_at",
    },
    ThesisResponse: {
        "id", "holding_id", "raw_input", "main_thesis", "key_assumptions",
        "positive_signals", "negative_signals", "key_risks", "confidence_score",
        "status", "created_at", "updated_at",
    },
    ThesisVersionResponse: {
        "id", "thesis_id", "version_no", "confidence_score", "status",
        "change_reason", "conflicting_assumptions", "observation_points",
        "snapshot", "created_at",
    },
    AnalysisResultResponse: {
        "id", "portfolio_id", "thesis_id", "analysis_type", "bull_summary",
        "bear_summary", "judge_summary", "concentration_theme", "concentration_score",
        "affected_holdings", "raw_result", "created_at",
    },
    AlertResponse: {
        "id", "user_id", "portfolio_id", "thesis_id", "severity", "title",
        "message", "is_sent", "sent_at", "created_at",
        # "delivery" is B-only (not in the frontend Alert type) — fine, extra field.
    },
    DashboardHoldingResponse: {
        "id", "portfolio_id", "ticker", "company_name", "quantity", "avg_buy_price",
        "target_weight", "current_weight", "created_at", "updated_at",
        "thesis", "latest_change",
    },
    PortfolioDashboardResponse: {
        "portfolio", "holdings", "concentration", "common_risks", "recent_alerts",
    },
    HoldingAnalysisResponse: {
        "thesis", "version", "evidence", "analysis_result", "alert",
    },
}

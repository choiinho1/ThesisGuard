...
News Agent는 SEC의 CIK·정식 회사명·티커·거래소·과거 회사명을 하나의 회사 식별 프로필로
만든 뒤 기본 검색과 핵심 가정별 검색을 병렬 실행한다.
경쟁·시장·실적 가정은 영어 금융 검색 힌트를 함께 사용하며, Bing News의 상세 RSS 설명을
우선 사용하고 결과가 없을 때 Google News로 대체한다. 공개 접근 가능한 발행사 페이지는
본문을 정제·축약해 사용하고, 동적 페이지·차단·유료벽이면 RSS 설명으로 대체한다. Source Selector는
회사 식별 결과를 `MATCH`, `AMBIGUOUS`, `MISMATCH`로 판정하고 `MATCH`만 다음 단계로 보낸다.
정식명·충분히 고유한 별칭·명시적 거래소/티커 표기·공식 도메인은 강한 식별 근거로 사용하고,
짧거나 일반명사인 회사명은 티커·산업·시장 문맥 등 추가 근거가 있어야 통과한다. 판정 결과와
사용한 신호는 문서 메타데이터에 기록한다. 이후 뉴스 최신성, URL/제목 중복, Thesis 키워드 일치를
모델 호출 없이 계산하며 기본 점수
...
3. CIK·티커·거래소·정식 법인명을 붙인 제목과 본문을 함께 임베딩하고, Dense cosine 검색과
   한국어/영어 BM25 검색 결과를 RRF로 결합한다.
...
- 문서 전체를 읽은 뒤 모든 핵심 가정을 SUPPORT/CONTRADICT/NOT_ADDRESSED로 각각
...
- 모델은 관련도와 영향도를 출력하지 않는다. 유효한 방향성 인용은 코드가 관련도 1.0으로,
  그 외에는 0.0으로 정한다. 영향도는 출처 유형의 고정 정책으로 정한다.
- SUPPORT/CONTRADICT이면서 Impact가 MEDIUM/HIGH인 근거만 Bull/Bear에 전달한다.
- 점수 엔진은 모든 정보×가정 노드 조합을 만들고 코드가 정한 방향×영향도×관련도로 signed strength를

...
- C는 B가 만든 `mcp_tools/`의 함수를 LangChain `@tool`로 감싸 사용 — 직접 외부 API를 호출하지 않는다
...
**`evidence_impact`** — 분류(지지/반박 여부)와 별개로 그 증거가 판단에서 차지하는 **비중/무게**
- `HIGH` / `MEDIUM` / `LOW` — 예: 똑같이 `CONTRADICT`라도 "CEO가 지나가며 한 말"은 LOW, "실적발표 가이던스 하향"은 HIGH

**`evidence_source_type`** — 증거를 어디서 가져왔는지 (5.5절: 출처마다 검색 방식이 다름 — RAG vs API)
- `SEC_FILING` — 미국 증권거래위원회 공시(10-K, 10-Q 등)
- `IR` — 기업이 직접 배포하는 투자자 대상 자료
- `EARNINGS` — 실적발표 자료 / 컨퍼런스콜
- `NEWS` — 뉴스 기사
...
| id | UUID PK | - |
| thesis_id | UUID FK → theses.id | - |
| source_type | evidence_source_type | SEC_FILING / IR / EARNINGS / NEWS / MACRO |
| source_url | TEXT (nullable) | 원문 링크 |
| vector_doc_id | VARCHAR (nullable) | Vector Store 문서 참조 ID |
| content_snippet | TEXT | 근거 원문 발췌 |
| classification | evidence_classification | SUPPORT / CONTRADICT / NEUTRAL / UNCERTAIN |
| impact | evidence_impact | HIGH / MEDIUM / LOW |
